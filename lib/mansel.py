...
class CheckableFileSystemModel(QtWidgets.QFileSystemModel):
    ...
    def __init__(
        ...
        if track_selection_size:
            ...
            if hasattr(parent, "finished"):
                ...
                print(
                    'CheckableFileSystemModel parent has no "finished" '
    ...
    def _handle_preselection(self, path: str) -> None:
        ...
        for row in range(self.rowCount(index)):
            ...
            child_path = Path(self.filePath(child)).relative_to(self.rootPath())
            status = self.preselection.check(child_path)
            if status == "leaf":
                self.setData(child, QtCore.Qt.Checked, QtCore.Qt.CheckStateRole)
    ...
    def _uncheck_exclusive_ancestors(self, index: QtCore.QModelIndex) -> None:
        ...
        while self.filePath(parent) != self.rootPath():
            child_data = (
                self._data(parent.child(i, index.column()))
            ...
            with os.scandir(ipath) as path_iter:
                for subpath in path_iter:
                    if subpath.is_dir():
                        ...
                        if pointer.walked:
                            self._track_item_size(
                                Path(path), ipath / subpath, pointer.size
                            )
                        else:
                            paths.append(subpath)
                    else:
                        self._track_item_size(
                            Path(path), ipath / subpath, subpath.stat().st_size
                        )
...
class UIDialog(QtWidgets.QDialog):
    ...
    def __init__(
        self,
        root_path: str,
        selection: t.List[str] = None,
        parent: QtCore.QObject = None,
    ) -> None:
        QtWidgets.QDialog.__init__(self, parent)

        self.model = CheckableFileSystemModel(self, preselection=selection)
    ...
    def indicate_calculating(self) -> None:
        ...
        human_size = locale.formattedDataSize(
            self.selection_size, format=locale.DataSizeSIFormat
        )
...
def parse_options(args_in=None):
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Select files and directories below path to be " "output as a list"
        )
    )
    parser.add_argument("--path", "-p", help="Root path", default=".")
    parser.add_argument(
        "--selection",
        "-s",
        type=str,
...
def main_dialog(args_in=None):
    ...
    dialog = UIDialog(root_path=path, selection=selection)
    dialog.show()
    return dialog
